...
Changes from the original ES3.g:
...
@lexer::members
{
    ...
    ECMAScript3Lexer.prototype.areRegularExpressionsEnabled = function() {
        if (last == null) {
            return true;
        }
	switch (last.getType())
	{
	// identifier
		case ECMAScript3Lexer.Identifier:
	// literals
		case ECMAScript3Lexer.NULL:
		case ECMAScript3Lexer.TRUE:
		case ECMAScript3Lexer.FALSE:
		case ECMAScript3Lexer.THIS:
		case ECMAScript3Lexer.OctalIntegerLiteral:
		case ECMAScript3Lexer.DecimalLiteral:
		case ECMAScript3Lexer.HexIntegerLiteral:
		case ECMAScript3Lexer.StringLiteral:
	// member access ending 
		case ECMAScript3Lexer.RBRACK:
	// function call or nested expression ending
		case ECMAScript3Lexer.RPAREN:
			return false;
	// otherwise OK
		default:
			return true;
    ...
    ECMAScript3Lexer.prototype.consumeIdentifierUnicodeStart = function() {
    ...
    ECMAScript3Lexer.prototype.nextToken = function() {
	var result = ECMAScript3Lexer.superclass.nextToken.call(this);
    ...
    ECMAScript3Lexer.prototype.isIdentifierPartUnicode = function(ch) {
    ...
    ECMAScript3Lexer.prototype.isIdentifierStartUnicode = function(ch) {
    ...
    ECMAScript3Lexer.prototype.isUnicodeLetter = function(ch) {
        if ((ch >= '\u0041' && ch <= '\u005A') ||
            (ch >= '\u0061' && ch <= '\u007A') ||
            (ch == '\u00AA') ||
            (ch == '\u00B5') ||
    ...
    ECMAScript3Lexer.prototype.isUnicodeCombiningMark = function(ch) {
        if (ch < '\u0300') {
            return false;
        }
        else {
    ...
    ECMAScript3Lexer.prototype.isUnicodeDigit = function(ch) {
        if (ch >= '\u0030' && ch <= '\u0039') {
            return true;
        }
        else if (ch < '\u0660') {
    ...
    ECMAScript3Lexer.prototype.isUnicodeConnectorPunctuation = function(ch) {
        if (ch == '\u005F') {
            return true;
        }
        else if (ch < '\u203F') {
...
@parser::members
{
    ...
    ECMAScript3Parser.prototype.isLeftHandSideAssign = function(lhs, cached) {
	if (cached.length != 0) {
            return cached[0];
	}
    ...
    ECMAScript3Parser.prototype.isLeftHandSideExpression = function(lhs) {
    ...
    ECMAScript3Parser.prototype.isLeftHandSideIn = function(lhs, cached) {
	if (cached.length != 0) {
            return cached[0];
	}
	
	var result = this.isLeftHandSideExpression(lhs) && (this.input.LA(1) == ECMAScript3Parser.IN);
	cached[0] = result;
	return result;
    }

    ECMAScript3Parser.prototype.promoteEOL = function(rule) {
	...
	if (!(la == ECMAScript3Parser.SEMIC || la == ECMAScript3Parser.EOF || la == ECMAScript3Parser.RBRACE || la == ECMAScript3Parser.EOL || la == ECMAScript3Parser.MultiLineComment)) {
            ...
            for (var ix = lt.getTokenIndex() - 1; ix > 0; ix--) {
                ...
                else if (lt.getType() == ECMAScript3Parser.EOL || (lt.getType() == ECMAScript3Parser.MultiLineComment && lt.getText().matches("/.*\r\n|\r|\n"))) {
...
arrayItem
	: ( expr=assignmentExpression | { this.input.LA(1) == ECMAScript3Parser.COMMA }? )
	-> ^( ITEM $expr? )
	;
...
*/
postfixExpression
	: leftHandSideExpression { if (this.input.LA(1) == INC || this.input.LA(1) == DEC) this.promoteEOL(null); } ( postfixOperator^ )?
	;
...
options
{
	...
	: { this.input.LA(1) == ECMAScript3Parser.LBRACE }? block
	| statementTail
	;
...
// The predicate is there just to get rid of the warning. ANTLR will handle the dangling else just fine.
	: IF LPAREN expression RPAREN statement ( { this.input.LA(1) == ECMAScript3Parser.ELSE }? ELSE statement )?
...
*/
continueStatement
	: CONTINUE^ { if (this.input.LA(1) == ECMAScript3Lexer.Identifier) this.promoteEOL(null); } Identifier? semic!
	;
...
*/
breakStatement
	: BREAK^ { if (this.input.LA(1) == ECMAScript3Lexer.Identifier) this.promoteEOL(null); } Identifier? semic!
	;
...
options
{
	...
	: { this.input.LA(1) == ECMAScript3Lexer.FUNCTION }? functionDeclaration
	| statement
	;
